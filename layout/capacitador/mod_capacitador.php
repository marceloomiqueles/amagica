...
if (strlen($nombre) > 0 && strlen($apellido) && $sexo > 0 && strlen($fono) == 12 && $id_box > 0) {
	$cambios = array($nombre, $apellido, $sexo, 5, $fono, 1, 0, 0);
	if ($cliente->actualiza_usuario_id($cambios, $id_box))
		header("Location: ver_capacitador.php?usr=" . $id_box . "&exito=2");
	else
		header("Location: mod_capacitador.php?usr=" . $id_box . "&exito=3");
}
