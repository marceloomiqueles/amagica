...
$okpost = 0;
if (isset($_GET['solic']) && isset($_GET['lang'] ) && isset($_GET['curso'])) {
	$solic = $_GET['solic'];
...
if ($okpost == 1) {
	...
	$clientetxt = "cliente2014"; //$solic;
	if (strcasecmp($clientetxt, "") == 0) {
		echo "Valor de marca esta vacio";
		$clientetxt = 0;
		exit();
	}
	$htmls = array();
	...
	for ($i = 1; $i < 12; $i++) {
		...
		foreach ($files as $text) {
			...
			$rep = $match[1];
			if (strcasecmp($rep, "") == 0) {
				echo "cliente sin texto ";
				$st2 = str_replace('var cliente = ""', 'var cliente = "' . $clientetxt . '"', $stemp);
			} else {
